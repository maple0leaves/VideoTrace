...
public class MinioUtils {
    ...
    private final String playbackProxyPrefix;

    public MinioUtils(MinioClient minioClient,
                      @Value("${minio.bucketName}") String bucketName,
                      @Value("${minio.endpoint}") String endpoint,
                      @Value("${minio.playback-proxy-prefix:}") String playbackProxyPrefix) {
        this.minioClient = minioClient;
        this.bucketName = bucketName;
        this.endpoint = endpoint.replaceAll("/+$", "");
        this.playbackProxyPrefix = normalizeProxyPrefix(playbackProxyPrefix);
    }
    ...
    public String playbackSource(String source) {
        if (!isManagedFile(source)) return source;
        return proxyPlaybackUrl(readableSource(source));
    }
    ...
    private String proxyPlaybackUrl(String presignedUrl) {
        if (playbackProxyPrefix.isBlank()) return presignedUrl;
        URI uri = URI.create(presignedUrl);
        String query = uri.getRawQuery();
        return playbackProxyPrefix + uri.getRawPath()
                + (query == null || query.isBlank() ? "" : "?" + query);
    }

    private String normalizeProxyPrefix(String prefix) {
