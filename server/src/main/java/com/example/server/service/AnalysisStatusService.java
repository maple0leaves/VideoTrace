...
public class AnalysisStatusService {
    ...
    public TaskStatus current(Long mediaId, String goal) {
        ...
        if (dispatchService.isActive(mediaId, goal)) {
            TaskStatus.State state = stage == null || stage == TaskStage.QUEUED
                    ? TaskStatus.State.QUEUED : TaskStatus.State.PROCESSING;
        ...
        if (stage == TaskStage.CANCELLED) {
            return TaskStatus.of(TaskStatus.State.CANCELLED, "分析任务已取消");
        }
        if (stage == TaskStage.TIMED_OUT) {
            return TaskStatus.of(TaskStatus.State.FAILED, "分析超过总时限，已自动终止；可稍后重试");
        }
