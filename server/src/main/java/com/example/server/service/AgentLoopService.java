...
public class AgentLoopService {
    ...
    public AgentState run(Long mediaId, VideoContext context, String runId) {
        if (runId != null) activeRunId.set(runId);
        try {
            return runControlled(mediaId, context);
        } finally {
            if (runId != null) activeRunId.remove();
        }
    }

    private AgentState runControlled(Long mediaId, VideoContext context) {
        checkCancelled();
    ...
    private void checkCancelled() {
        ...
        if (runId != null) taskControlService.abortIfRequested(runId);
    }
